...
import { useRouter, usePathname } from 'next/navigation' // usePathname for App Router but keeping useRouter for compatibility if needed or migrating completely
...
function MobileNavItem({ href, children, icon: Icon }: MobileNavItemProps) {
  return (
    <li>
...
function NavItem({ href, children, icon: Icon }: NavItemProps) {
  ...
  return (
    <>
      ...
      {isHomePage && (
        <div
          className="flex-none"
          style={{ height: "var(--header-height)" as any }}
        />
      )}
    </>
  )
